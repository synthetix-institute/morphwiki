...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {'schema_version':1,'report_type':'morphwiki_quantum_v2_evidence_index_audit','readiness':'usable' if pages else 'blocked','index':args.index,'tree':args.tree,'coverage':idx.get('coverage') or {},'status_counts':dict(statuses),'tree_pages_missing_from_index':sorted(slugs-set(pages))[:50],'pages_without_v2_source_grounding_count':len(no_v2),'pages_without_v2_source_grounding_examples':no_v2[:40],'top_tokens':toks.most_common(30),'top_constructor_roles':roles.most_common(20),'top_routes':routes.most_common(20),'conclusions':['Use the index as evidence routing, not as public language.','Pages without V2 source grounding should remain backed by legacy witnesses until full source-card alignment is transferred.','The book should translate supported roles into ordinary quantum language and avoid exposing internal token names by default.'],'claim_scope':'Audit of the MorphWiki V2 evidence adapter. It measures grounding coverage and token/role support; it does not validate equation derivations.'}
