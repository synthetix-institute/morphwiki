...
if command -v latexmk >/dev/null 2>&1 && command -v pdflatex >/dev/null 2>&1; then
  echo "[MorphWiki] compiling PDF with latexmk/pdflatex"
  latexmk -pdf -interaction=nonstopmode -halt-on-error -outdir="$OUT_DIR" "$TEX"
elif command -v pdflatex >/dev/null 2>&1; then
  echo "[MorphWiki] compiling PDF with pdflatex"
  pdflatex -interaction=nonstopmode -halt-on-error -output-directory "$OUT_DIR" "$TEX"
  pdflatex -interaction=nonstopmode -halt-on-error -output-directory "$OUT_DIR" "$TEX"
