...
Quantum number is placed at the state step, where the carrier of prediction is specified: the object that is propagated, transformed, restricted, or measured.
...
The state carrier is a state vector, wave function, density operator, field state, register state, or superselection sector. The calculation involves the transformations and observables defined on that state space. Normalization, positivity, inner-product, tensor-factorization, and superselection conditions define the allowed states. The calculated quantities include Born probabilities, expectation values, reduced states, or correlation functions determined by the state.
